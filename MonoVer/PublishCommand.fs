...
let ParseChangeset (fileInfo: FileInfo): Result<Changeset, PublishDomainErrors> =
    ...
    match (ChangesetParser.Parse content) with
        ...
        | Ok cs -> Ok {Id = (ChangesetId.Id fileInfo.Name); Content = cs}
       
       
let CHANGELOG_TEMPLATE = """All notable changes to this project will be documented in this file.
...
let ReadChangelogFor (csproj: FileInfo): Changelog  =
    {
     ...
     Content = match csproj.Directory.GetFiles("Changelog.md")|> Seq.tryHead with
...
let RunPublish (args: PublishOptions) : Result<unit, PublishDomainErrors> =
    ...
    // get all changesets
    monad {
        let! changesets = DirectoryInfo(changesets)
                             .EnumerateFiles("*.md")
    ...
    // get all projects
        let! projects =
            ...
            | [|x|] -> Ok (SlnParser.getProjects( SolutionFile.Parse(x)))
            | files -> Error (MultipleSolutionFilesFoundInWorkdir files)
    
    // publish
        let res =  Publish projects changesets
    // update version 
        for change in res do
            UpdateCsproj.ApplyChanges change
             
    // update/create Changelogs
        let changelogs  =
            res
            |> List.map (fun x -> (ReadChangelogFor x.Project.Csproj, {Version = x.NextVersion; Date = DateOnly.FromDateTime DateTime.Today;Changes = x.Changes }))
