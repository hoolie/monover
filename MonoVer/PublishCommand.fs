...
let private loadSolution workdir =
    MsProjects.TryLoadFrom workdir |> Result.mapError MsProjectsError
...
let private processChangesets versionSuffix projects =
    (Changesets.Publish projects versionSuffix)
    >> (Result.mapError PublishDomainErrors.PublishError)

let private updateChangelog
    (solution: MsProjects.MsSolution)
    ({ Project = project
       Changes = changes
       Version = version }: NewChangelogEntry)
    =
    let project =
    ...
    let changelogEntry: ChangelogVersionEntry =
        { Version = version
...
let RunPublish (args: PublishOptions) : Result<unit, ApplicationError> =
    ...
    let versionSuffix = VersionSuffix args.VersionSuffix
    let processChangesets = processChangesets versionSuffix
    ...
    |> Result.mapError (function
        ...
        | MsProjectsError(MultipleSolutionFilesFoundInWorkdir x) ->
            CommandError(-6, $"""Found multiple solution file in working directory: {x}""")
        | UpdateVersionError(FailedToUpdateVersionInFile e) ->
            CommandError(-7, $"Failed to update file {e.Project} to version {Version.toString e.Version} "))
