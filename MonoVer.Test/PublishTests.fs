...
let mockChangeset id content : RawChangeset =
    ( (Id id), content)
...
"""
    ...
    let value = shouldBeOk result
    match value with
    | [NewChangelogEntry newChangelog;VersionIncreased version; ChangesetApplied _] ->
        newChangelog.Project.Name |> should equal "TestProject"
        version.Version |> should equal (Version.FromString("1.1.0"))
    |_ -> failwith "did not receive expected events"
